...
def smooth_keypoints(buffer, window_size=5):
    """
    Aplica un suavizado por promedio móvil a los keypoints en el buffer.

    Args:
        buffer (list): Buffer con los keypoints a suavizar.
        window_size (int): Tamaño de la ventana para el promedio móvil.

    Returns:
        np.ndarray: Buffer suavizado.
    """
    buffer_array = np.array(buffer)
    if buffer_array.shape[0] < window_size:
        return buffer_array  # Devuelve el buffer sin cambios si es más pequeño que la ventana

    smoothed = []
    for i in range(buffer_array.shape[1]):  # Itera sobre cada característica
        smoothed_feature = np.convolve(buffer_array[:, i], np.ones(window_size) / window_size, mode='valid')
